...
def preprocess_data(data):
    """
    数据预处理：将 Modal Type 转换为数值标签，标准化特征数据，并转换为 LSTM 所需的格式。
    """
    ...
    # 将数据转换为 LSTM 所需的 3D 格式 [samples, timesteps, features]
    X_train_lstm = X_train.reshape((X_train.shape[0], 1, X_train.shape[1]))
    X_test_lstm = X_test.reshape((X_test.shape[0], 1, X_test.shape[1]))

    return X_train_lstm, X_test_lstm, y_train, y_test, label_encoder, scaler, features, target
...
def build_lstm_model(input_shape, num_classes):
    ...
    """
    model = Sequential()
    model.add(Input(shape=input_shape))  # 添加 Input 层作为第一层
    model.add(LSTM(64, return_sequences=False))  # LSTM 层
...
def main():
    ...
    # 数据预处理
    X_train_lstm, X_test_lstm, y_train, y_test, label_encoder, scaler, features, target = preprocess_data(data)
    ...
    # 训练模型
    train_model(model, X_train_lstm, y_train, X_test_lstm, y_test)

    # 评估模型并保存结果
    evaluate_and_save_results(model, X_test_lstm, y_test, label_encoder, X_test_original, y_test_original)

    # 保存模型
    model.save('lstm_model.h5')
    print("LSTM 模型已保存为 lstm_model.h5")
